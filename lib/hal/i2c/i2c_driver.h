...
#include "general/common/lib_base.h"
#include "general/common/error.h"
...
typedef struct {
    ...
    uint8_t *tx_data; // Transmit data buffer (NULL to disable transmit)
...
extern error_t i2c_transfer(handle_t handle, i2c_transaction_t *transaction, uint32_t retries, timer_t timeout);
